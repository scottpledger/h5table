...
var MultipleSelection = require('h5table/selection/multiple');
...
test('empty update clears the selection', function (assert) {
  ...
  var $rendered = $selection.find('.h5table-selection__rendered');
...
test('clear button respects the disabled state', function (assert) {
  ...
  var $pill = $rendered.find('.h5table-selection__choice');

  assert.equal($pill.length, 1, 'There should only be one selection');

  var $remove = $pill.find('.h5table-selection__choice__remove');
